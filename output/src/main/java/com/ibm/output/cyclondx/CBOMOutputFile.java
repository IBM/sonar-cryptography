...
import com.ibm.output.cyclondx.builder.ProtocolProperties17;
        ...
        if (protocolDependency != null) {
            ...
            if (!cryptoRefs.isEmpty()
                    ...
                    && protocol.getCryptoProperties().getProtocolProperties()
                            instanceof ProtocolProperties17 protoProps17) {
                List<RelatedCryptoMaterialProperties> relatedAssets = new ArrayList<>();
                for (String ref : cryptoRefs) {
                    RelatedCryptoMaterialProperties rcm = new RelatedCryptoMaterialProperties();
                    rcm.setId(ref);
                    rcm.setType(RelatedCryptoMaterialType.OTHER);
                    relatedAssets.add(rcm);
                }
                protoProps17.setRelatedCryptographicAssets(relatedAssets);
            }
        }
    }
