...
/**
 * Verifies that transitive alias chains are resolved correctly.
 *
 * <p>Test scenario (from ProbeCSharpChainedAliasTestFile.cs):
 *
 * <pre>{@code
 * var aes = Aes.Create();
 * var alias1 = aes;
 ...
 * }</pre>
 *
 * <p>When {@code alias2} is used for property setters through a chain of aliases, the engine should
 * resolve the chain transitively back to {@code aes} and link the property values to the original
 * detection.
 */
class DotNetAESChainedAliasTest extends TestBase {

    @Test
    void testChainedAlias() throws Exception {
        CSharpVerifier.verify("rules/detection/dotnet/ProbeCSharpChainedAliasTestFile.cs", this);
    }
    ...
    public void asserts(
        ...
        switch (findingId) {
            case 0 -> {
                /*
                 * TestViaChainedAlias: var aes = Aes.Create() + chain of aliases + setters
